...
class PostCommentsRequest {
    ...
    fun request(doc: Document) {
        ...
        for (node in doc.select("div.comment")) {
            ...
            val parentId = if ("comment_list" == parent.className()) PostCommentsRequest.NumberExtractor[parent.id()] else 0
        ...
        PostCommentsRequest.ChildrenCounter.compute(result)
        comments = result
    }
    ...
    private object NumberExtractor {
        ...
        internal operator fun get(text: String): Long {
            val m = PostCommentsRequest.NumberExtractor.NUMBER.matcher(text)
